...
def apply_right_alignment_filters(vf_right, alignment):
    if not alignment:
        return

    rotate_deg = float(alignment.get("right_rotate_deg", 0.0))
    shift_x = int(round(float(alignment.get("right_shift_x_px", 0.0))))
    shift_y = int(round(float(alignment.get("right_shift_y_px", 0.0))))

    if abs(rotate_deg) > 1e-9:
        rotate_rad = rotate_deg * np.pi / 180.0
        vf_right.append(f"rotate={rotate_rad:.12f}:ow=iw:oh=ih:c=black")
...
def build_render_command(
    ...
    apply_right_alignment_filters(vf_right, alignment)
...
def print_manual_alignment(alignment):
    ...
    print(f"RIGHT X shift:              {alignment['right_shift_x_px']:+.3f} px")
    print(f"RIGHT Y shift:              {alignment['right_shift_y_px']:+.3f} px")
    print(f"RIGHT roll rotation:        {alignment['right_rotate_deg']:+.3f} deg")
...
def main():
    ...
    parser.add_argument(
        "--stereo-output",
        choices=["sbs", "anaglyph"],
        default="sbs",
        help="Output stereo format. 'sbs' = side-by-side, 'anaglyph' = red/cyan style single-frame 3D."
    )
    parser.add_argument(
        "--anaglyph-mode",
    ...
    alignment = {
        "right_shift_x_px": float(args.manual_right_shift_x),
        "right_shift_y_px": float(args.manual_right_shift_y),
        "right_rotate_deg": float(args.manual_right_rotate_deg),
    }
    ...
    if args.auto_align_vertical or args.auto_align_horizontal:
        ...
        if args.auto_align_horizontal:
            alignment["right_shift_x_px"] += auto_alignment["suggested_right_shift_x_px"]
        if args.auto_align_vertical:
            alignment["right_shift_y_px"] += auto_alignment["suggested_right_shift_y_px"]
